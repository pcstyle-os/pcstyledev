...
import { Terminal, Copy, X, Check, Send, Loader2, AlertCircle } from "lucide-react";
...
export function ContactModal({ isOpen, onClose }: ContactModalProps) {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>("ssh");
  ...
  // focus first field when switching to web form
  useEffect(() => {
    if (isOpen && viewMode === "web" && firstInputRef.current) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      setSubmitMessage(translations.contact.errors.fixErrors);
        ...
        <>
          ...
          >
            <div className="brutal-border brutal-shadow relative overflow-hidden rounded-[var(--radius-card)] bg-[var(--color-background)] p-8">
              ...
              {/* tab bar */}
              <div className="mb-6 flex gap-2 border-b-4 border-[var(--color-ink)]">
                <button
                  onClick={() => setViewMode("ssh")}
                  className={`px-6 py-3 font-semibold uppercase tracking-wide transition-all ${
                    viewMode === "ssh"
                      ? "border-b-4 border-[var(--color-cyan)] bg-[var(--color-cyan)]/10 text-[color:var(--color-ink)]"
                      : "text-[color:var(--color-ink)]/60 hover:text-[color:var(--color-ink)]"
                  }`}
                >
                  {translations.contact.sshTab}
                </button>
                <button
                  onClick={() => setViewMode("web")}
                  className={`px-6 py-3 font-semibold uppercase tracking-wide transition-all ${
                    viewMode === "web"
                      ? "border-b-4 border-[var(--color-magenta)] bg-[var(--color-magenta)]/10 text-[color:var(--color-ink)]"
                      : "text-[color:var(--color-ink)]/60 hover:text-[color:var(--color-ink)]"
                  }`}
                >
                  {translations.contact.webTab}
                </button>
              </div>
                ...
                >
                  <div className="mb-6">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h2 id="contact-modal-title" className="text-3xl font-black uppercase text-[color:var(--color-ink)] sm:text-4xl">
